    ...
    {!isLocked && (
      <div className="flex items-center space-x-3 opacity-0 group-hover:opacity-80 transition-opacity duration-300">
        <FaPencil
          ...
          className="w-3.5 h-3.5 text-yellow-600 cursor-pointer hover:text-yellow-500"
          title="Editar"
        />
        <FaPlus
          onClick={(e) => { e.stopPropagation(); onAddVersion(); }}
          className="w-3.5 h-3.5 text-green-600 cursor-pointer hover:text-green-500"
...
const EditItemForm: React.FC<EditItemFormProps> = ({ initialName, initialUrl, onSave, onCancel }) => {
  ...
  const handleSave = () => onSave(name, url);
...
const ITTOListItem: React.FC<ITTOListItemProps> = ({
  item,
  isEditing,
  processType,
  onEditStart,
          ...
          <>
            ...
            <div className="flex-shrink-0">
              <ActionIcons
                ...
                onDelete={() => {
                  if (activeVersion) {
                    onDeleteRequest(activeVersion.id, cleanName, item.id);
                  } else {
                    onDeleteRequest(item.id, cleanName);
                  }
                }}
            ...
            {allSelectableVersions.map(version => (
              <li key={version.id} className="text-sm text-gray-800 flex items-center group/version justify-between py-1">
                <div className="flex items-center">
                  <span
                    className={!isLocked ? 'cursor-pointer p-1 -ml-1 mr-2' : 'p-1 -ml-1 mr-2'}
                    onClick={(e) => { e.stopPropagation(); if (!isLocked) onSelectVersion(item.id, version.id); }}
                  >
                    {version.isActive
                      ? <FaCircleCheck className="w-4 h-4 text-green-500" title="Versión activa" />
                      : <FaRegCircle className={`w-4 h-4 text-gray-400 ${!isLocked ? 'group-hover/version:text-green-500 transition-colors' : 'cursor-not-allowed'}`} title={!isLocked ? 'Activar esta versión' : 'No se puede cambiar la versión en progreso'} />
                    }
                  </span>
                  <FaFile className="w-3 h-3 mr-2 text-gray-400 flex-shrink-0" />
                  {version.url ? (
