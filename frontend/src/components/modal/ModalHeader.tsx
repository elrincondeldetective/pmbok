...
import type { AnyProcess, KanbanStatus, IPMBOKProcess, IScrumProcess } from '../../types/process';
...
const ModalHeader: React.FC<ModalHeaderProps> = ({ process, onClose, onKanbanStatusChange }) => {
        ...
        <div className={`p-6 rounded-t-xl ${process.status?.tailwind_bg_color || 'bg-gray-700'} ${process.status?.tailwind_text_color || 'text-white'}`}>
            <div className="flex justify-between items-start gap-4">
                ...
                <div className="flex-shrink-0 flex items-center gap-4">
                    ...
                    <button onClick={onClose} className="text-2xl font-bold hover:opacity-75 transition-opacity" aria-label="Cerrar modal">
                        &times;
                    </button>
                </div>
            </div>
