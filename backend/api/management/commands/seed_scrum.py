...
from api.models import ProcessStatus, ProcessStage, ScrumProcess
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # --- Estatus y Etapas de Scrum con sus colores correctos ---
        
        # Estatus genérico para todos los procesos de Scrum
        ...
        # Etapas de Scrum, replicando los colores del HTML
        stage_inicio, _ = ProcessStage.objects.get_or_create(
            name="Inicio (Scrum)", 
            defaults={'tailwind_bg_color': 'bg-sky-100', 'tailwind_text_color': 'text-sky-800'}
        )
        stage_plan, _ = ProcessStage.objects.get_or_create(
            name="Planificación y Estimación (Scrum)", 
            defaults={'tailwind_bg_color': 'bg-amber-100', 'tailwind_text_color': 'text-amber-800'}
        )
        stage_impl, _ = ProcessStage.objects.get_or_create(
            name="Implementación (Scrum)", 
            defaults={'tailwind_bg_color': 'bg-green-100', 'tailwind_text_color': 'text-green-800'}
        )
        stage_retro, _ = ProcessStage.objects.get_or_create(
            name="Revisión y Retrospectiva (Scrum)", 
            defaults={'tailwind_bg_color': 'bg-indigo-100', 'tailwind_text_color': 'text-indigo-800'}
        )
        stage_lanz, _ = ProcessStage.objects.get_or_create(
            name="Lanzamiento (Scrum)", 
            defaults={'tailwind_bg_color': 'bg-rose-100', 'tailwind_text_color': 'text-rose-800'}
        )
        stage_grandes_proyectos, _ = ProcessStage.objects.get_or_create(
            name="Scrum para grandes proyectos",
            defaults={'tailwind_bg_color': 'bg-slate-200', 'tailwind_text_color': 'text-slate-800'}
        )
        stage_empresa, _ = ProcessStage.objects.get_or_create(
        ...
        self.stdout.write('Seeding all 27 Scrum processes...')
        for num, name, inputs, tools, outputs, stage_obj in scrum_processes_data:
        ...
        self.stdout.write(self.style.SUCCESS('All 27 Scrum processes have been seeded successfully!'))
